...
def retrieve_image(ra, dec, size, res, logger, options=None):
    ...
    def SDSS_coverage(ra, dec, logger):
        ...
        request_sdssCoverage = urlencode({"coverageRA": ra, "coverageDec": dec})
    ...
    def blank_image(size_pix, scale):
        ...
        CD = numpy.matrix([[-1.0 * scale / 3600.0, 0], [0, 1.0 * scale / 3600.0]])
        return imarray, CD, "", ""
    ...
    def retrieve_image_PS1(ra, dec, size, scale, logger):
        ...
        size_pix = int(size * 3600.0 / scale) * scale_factor
        ...
        try:
            ...
            logger.warning(
                "HTTP ERROR request of: %s %s", url_ps1_fitscut, request_cutout
            )
            return blank_image(size_pix / scale_factor, scale)
        ...
        for c in range(nc):
            ...
            rgb_arr.append(
                numpy.array(
                    255 * (1.0 - binned / numpy.max(numpy.array(binned))), dtype=int
                )
            )

        # stick binned up channels into single RGB image
        rgb_binned = numpy.zeros(
            (rgb_arr[0].shape[0], rgb_arr[0].shape[1], 3), dtype=int
        )
        ...
        try:
            ...
            logger.warning("HTTP ERROR request of: %s %s", url_ps1_fitscut, request_wcs)
            return blank_image(size_pix / scale_factor, scale)

        try:
            wcs_dict = json.load(wcsr)
            CD = numpy.matrix(
                scale_factor * numpy.array(wcs_dict["cdmatrix"]).reshape(2, 2)
            )
        except KeyError:
    ...
    def retrieve_image_PANSTARRS(ra, dec, size, scale, logger):
        ...
        size_pix = int(size * 3600.0 / scale)
    ...
    def retrieve_image_SDSS(ra, dec, size, options, scale, logger):
        ...
        if size_pix > 2048:
            size_pix = 2048
            scale = size * 3600.0 / size_pix

        request_sdss = urlencode(
            {
                "ra": ra,
                "dec": dec,
                "scale": scale,
    ...
    def retrieve_image_SkyView(ra, dec, size, scale, logger):
        size_pix = min(512, int(size * 3600.0 / scale))
        scale = size * 3600.0 / size_pix

        paths = SkyView.get_images(
            position="%3.5f %2.5f" % (ra, dec),
            survey="DSS2 Blue",
            coordinates="J2000",
...
if __name__ == "__main__":
    ...
    ra, dec, size, options, scale = 0.0, 0.0, 3.0, "", 1.5
